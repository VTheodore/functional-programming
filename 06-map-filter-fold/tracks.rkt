...
(define (longer-tracks tracks)
  (filter (lambda (track) (> (/ (list-ref track 3) 1000) 240)) tracks)
)

; Да вземем само имената на горните песни?

(define (names-of-longer-tracks tracks)
  (map cadr (longer-tracks tracks))
)

; Да направим времетраенето на всяка песен в секунди
(define (duration-in-seconds tracks)
  (map (lambda (track) (list-set track 3 (floor (/ (list-ref track 3) 1000)))) tracks)
)

; Да намерим най-популярната песен

(define (most-popular-track tracks)
  (define (get-popularity track) (list-ref track 2))
  (foldl
   (lambda (curr res)
     (if (or (null? res) (> (get-popularity curr) (get-popularity res)))
         curr
         res)
   )
   '() tracks)
...
(define (average-popularity tracks)
  (/ (foldl (lambda (curr res) (+ res (list-ref curr 2))) 0 tracks) (length tracks))
)

; Всички песни с популярност под средната

(define average (average-popularity tracks))

(define (hipster-tracks tracks)
  (filter (lambda (track) (< (list-ref track 2) average)) tracks)
)

; Всички песни с популярност над средната

(define (mainstream-tracks tracks)
  (remove* (hipster-tracks tracks) tracks)
)
...
(define (combined-artist-and-track-name tracks)
  (map (lambda (track) (string-append (list-ref track 0) " - "(list-ref track 1))) tracks)
)